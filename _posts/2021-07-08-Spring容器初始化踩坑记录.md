...
​		整体思路是提前 `RestfulApiProxy`的初始化，有以下三种方式。
...
​		反复阅读代码，突然灵光一现，发现了问题所在。我在类A中注入类B，类A使用的是Spring的@Component注解，而同事是在Dubbo的@Service中注入的。那么到底有何不同？下面分析Dubbo的Service初始化过程。
...
![invokeBeanFactoryPostProcessors.BeanDefinitionRegistryPostProcessor](https://github.com/Michael1024-CJX/blog/blob/main/image/invokeBeanFactoryPostProcessors.BeanDefinitionRegistryPostProcessor.png) 
...
![image-20210709113408297](https://github.com/Michael1024-CJX/blog/blob/main/image/invokeBeanFactoryPostProcessors.BeanFactoryPostProcessor.png)		而BeanFactory维护了一个BeanDefinitionNames的List，List是有序的。这就意味着，在Spring初始化Bean的时候，会先初始化项目中定义的Bean，后初始化Dubbo Service Bean。这时 `RestfulApiProxy` 就能比Dubbo Service 初始化了，而它初始化后就会将代理类注入到Spring容器中，后面的DubboServiceBean就能够从Spring容器中获取接口的实现类了。
