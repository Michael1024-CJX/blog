...
![](D:\IDEAWorkSpace\cjx\blog\image\invokeBeanFactoryPostProcessors.BeanDefinitionRegistryPostProcessor.png) 
...
![image-20210709113408297](D:\IDEAWorkSpace\cjx\blog\image\invokeBeanFactoryPostProcessors.BeanFactoryPostProcessor.png)		而BeanFactory维护了一个BeanDefinitionNames的List，List是有序的。这就意味着，在Spring初始化Bean的时候，会先初始化项目中定义的Bean，后初始化Dubbo Service Bean。这时 `RestfulApiProxy` 就能比Dubbo Service 初始化了，而它初始化后就会将代理类注入到Spring容器中，后面的DubboServiceBean就能够从Spring容器中获取接口的实现类了。
